...
#include <pwx_worker_RNG.h> // Provides pwx::RNG
using pwx::RNG;

// We need this to be able to save our matter container in env:
#include <pwxTDoubleRing.h>

/// @brief simple typedef to shorten things a bit...
typedef pwx::TDoubleRing<CMatter> matCont_t;
    ...
    while ( env->screen->GetEvent( event ) ) {
        ...
        /// 2.: Check for window resizing and maintain aspect ratio
            ...
            if ( newAspect > oldAspect ) {
                ...
                newHeight = static_cast<uint32_t>( std::round( static_cast<double>( newWidth ) / oldAspect ) );
                ...
                newWidth = static_cast<uint32_t>( std::round( static_cast<double>( newHeight ) * oldAspect ) );
                env->screen->SetSize( newWidth, newHeight );
                env->screen->Display();
            }
...
// Without debugging on, the assert won't fail and we need the calculation here
    ...
    *toSleep = static_cast<int32_t>( std::round( mRatio * pMax / pRatio ) );
...
void showMsg( ENVIRONMENT* env, const char* fmt, ... ) {
    ...
    uint32_t bWidth = static_cast<uint32_t>( std::round( txtRect.GetWidth() + 4.f ) );
    uint32_t bHeight= static_cast<uint32_t>( std::round( txtRect.GetHeight() + 4.f ) );
    ...
    // Determine background box width
    bWidth = static_cast<uint32_t>( std::round( statRect.GetWidth()  + 4.f ) );
    bHeight= static_cast<uint32_t>( std::round( statRect.GetHeight() + 4.f ) );
                ...
                while ( dust ) {
                    ...
                    if ( ( dust->z > 0. ) && ( ( z < 0. ) || ( dust->z < z ) ) ) {
                        ...
                        if ( dustTran > 0.005 ) {
                            ...
                            r = static_cast<uint8_t>( std::round( ( static_cast<double>( r ) * dustTran )
                                                                 +( static_cast<double>( dust->r ) * dustOpaq ) ) );
                            g = static_cast<uint8_t>( std::round( ( static_cast<double>( g ) * dustTran )
                                                                 +( static_cast<double>( dust->g ) * dustOpaq ) ) );
                            b = static_cast<uint8_t>( std::round( ( static_cast<double>( b ) * dustTran )
...
void thrdInit( void* xEnv ) {
    ...
    */
     ...
     */
    double baseDist= (  std::pow( ( 3. * Base_Volume ) / ( 4. * M_PIl ), 1. / 3. ) // See CMatter::setRadius()
    ...
    if ( explode ) {
        ...
        */
        ...
        maxMov  = std::pow( ( 2. * Grav_Constant * M ) / ( maxDist / M_to_Pos ), 1. / 2. ); // This is the resulting m/s
    } else
